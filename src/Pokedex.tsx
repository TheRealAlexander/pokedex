    ...
    const pokemonData = await Promise.all(
      response.data.results.map(async (pokemon: PokemonResult) => {
        ...
        return {
          ...
          type2: pokemonResponse.data.types[1]?.type.name, // Fetch the second type if it exists
        };
      }),
    );
    ...
    if (event.target.value.trim() !== "") {
      if (isNaN(Number(event.target.value))) {
        ...
        while (!found) {
          ...
          const matchingPokemons = response.data.results.filter(
    ...
    <div className="pokedex-container">
      ...
      <div className="pokemon-grid">
        {filteredPokemon.map((pokemon) => {
          ...
          const pokemonColor =
            pokemonTypeColors[pokemon.type as keyof typeof pokemonTypeColors];
          const pokemonColor2 = pokemon.type2
            ? pokemonTypeColors[pokemon.type2 as keyof typeof pokemonTypeColors]
            : pokemonColor;
          ...
          return (
            <div
              ...
              style={{
                background: `linear-gradient(to right, ${pokemonColor} 50%, ${pokemonColor2} 50%)`,
              }}
            >
      ...
      <div className="pagination">
        {previous && !searchTerm && (
          <button
            ...
            onClick={() => fetchPokemons(previous)}
          >
            Previous
          </button>
        )}
